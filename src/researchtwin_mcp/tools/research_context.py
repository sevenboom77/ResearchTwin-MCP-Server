...
def get_research_context(store: JsonStore, *, project_name: str | None = None, recent_activity_limit: int = 5, recent_advisor_limit: int = 5, candidate_limit: int = 5, include_rejected: bool = False) -> dict[str, Any]:
    def action() -> dict[str, Any]:
        limits = (recent_activity_limit, recent_advisor_limit, candidate_limit)
        ...
        context = {"project_status": status_out, "recent_activities": activities[:recent_activity_limit], "recent_advisor_instructions": advisors[:recent_advisor_limit], "recent_candidates": candidates[:candidate_limit], "context_summary_fields": {"current_stage": status_out.get("current_stage") if status_out else None, "pending_tasks": status_out.get("pending_tasks", []) if status_out else [], "risks": status_out.get("risks", []) if status_out else [], "important_decisions": status_out.get("important_decisions", []) if status_out else []}}
...
def register_research_context_tool(server: MCPServer, store: JsonStore) -> None:
    ...
    def tool(project_name: NonEmptyText | None = None, recent_activity_limit: CandidateLimit = 5, recent_advisor_limit: CandidateLimit = 5, candidate_limit: CandidateLimit = 5, include_rejected: bool = False) -> Annotated[CallToolResult, GetResearchContextSuccess]:
        payload = get_research_context(store, project_name=project_name, recent_activity_limit=recent_activity_limit, recent_advisor_limit=recent_advisor_limit, candidate_limit=candidate_limit, include_rejected=include_rejected)
