...
from researchtwin_mcp.tools.project_knowledge import list_project_knowledge
if TYPE_CHECKING:
    from mcp.server import MCPServer

def get_research_context(store: JsonStore, *, project_name: str | None = None, recent_activity_limit: int = 5, recent_advisor_limit: int = 5, candidate_limit: int = 5, include_rejected: bool = False, recent_brief_limit: int = 3, recent_project_knowledge_limit: int = 3) -> dict[str, Any]:
    def action() -> dict[str, Any]:
        limits = (recent_activity_limit, recent_advisor_limit, candidate_limit, recent_brief_limit, recent_project_knowledge_limit)
        ...
        knowledge = list_project_knowledge(store, project_name=selected_name, limit=recent_project_knowledge_limit)
        knowledge_summaries = [{k: x.get(k) for k in ("knowledge_id", "candidate_id", "title", "knowledge_type", "sync_status", "source_url", "created_at", "synced_at")} for x in knowledge.get("knowledge", [])] if knowledge.get("status") == "success" else []
        context = {"project_status": status_out, "recent_activities": activities[:recent_activity_limit], "recent_advisor_instructions": advisors[:recent_advisor_limit], "recent_candidates": candidates[:candidate_limit], "recent_intelligence_briefs": brief_summaries, "recent_project_knowledge": knowledge_summaries, "context_summary_fields": {"current_stage": status_out.get("current_stage") if status_out else None, "pending_tasks": status_out.get("pending_tasks", []) if status_out else [], "risks": status_out.get("risks", []) if status_out else [], "important_decisions": status_out.get("important_decisions", []) if status_out else []}}
...
def register_research_context_tool(server: MCPServer, store: JsonStore) -> None:
    ...
    def tool(project_name: NonEmptyText | None = None, recent_activity_limit: CandidateLimit = 5, recent_advisor_limit: CandidateLimit = 5, candidate_limit: CandidateLimit = 5, include_rejected: bool = False, recent_brief_limit: CandidateLimit = 3, recent_project_knowledge_limit: CandidateLimit = 3) -> Annotated[CallToolResult, GetResearchContextSuccess]:
        payload = get_research_context(store, project_name=project_name, recent_activity_limit=recent_activity_limit, recent_advisor_limit=recent_advisor_limit, candidate_limit=candidate_limit, include_rejected=include_rejected, recent_brief_limit=recent_brief_limit, recent_project_knowledge_limit=recent_project_knowledge_limit)
